    ...
    <?php if ($rows): ?>
        ...
        <?php
            ...
            <div id="dash_welcome" class="card white mb-5">
                <?php $nid = viamo_standard_account_get_nid_by_host_id($rows[0]['uid']); ?>
                <?php if ($nid > 0) { ?>
                    <div class="card-image">                    
                        <a  href="node/<?php print $nid; ?>/edit"  class="btn-floating halfway-fab waves-effect waves-light v-gold tooltipped" data-position="left" data-delay="50" data-tooltip="<?php print "Edit details"; ?>"><i class="material-icons"><?php print t('edit'); ?></i></a>
                    </div>           
                <?php } ?>                                
                <div class="card-content">
                    <span class="card-title v-gold-text"><?php print t('Manage your Wedding Day'); ?></span></span>
                    <p class="mb-3"><?php print isset($rows[0]['body']) ? $rows[0]['body'] : ' '  ?></p>                                    
                    <dl class="row">
                ...
                <?php
                        /* below function use for event exist or not for login uses.*/                         
                        $nid = viamo_standard_account_get_nid_by_host_id($rows[0]['uid']);
                        if ($nid > 0) {
                        // echo "<pre>";
                        // print_r($rows); exit;
                        ?>
                            <dt class="col s12 m3 bold"><?php print t('Type Of Ceremeny'); ?></dt>
                            <dd class="col s12 m9 v-gold-text">                                        
                                <?php echo isset($rows[0]['field_type_of_ceremeny']) ? $rows[0]['field_type_of_ceremeny'] : '--' ?>                    
                            </dd>
                            ...
                            <?php if ($rows[0]['title_2'] && $rows[0]['field_other_ceremony_venue_on_of'] == 0): ?>
                                <dt class="col s12 m3 bold"><?php print t('Ceremony'); ?></dt>
                                <dd class="col s12 m9 v-gold-text"><?php print isset($rows[0]['title_2']) ? $rows[0]['title_2'] . ' at ' . $rows[0]['field_ceremony_time'] : ' '  ?></dd>
                            <?php else: ?>
                                <dt class="col s12 m3 bold"><?php print t('Ceremony'); ?></dt>
                                <dd class="col s12 m9 v-gold-text"><?php print isset($rows[0]['field_other_ceremony_venue']) ? $rows[0]['field_other_ceremony_venue'] . ' at ' . $rows[0]['field_ceremony_time'] : ' '  ?></dd>
                            <?php endif; ?>  

                            <?php if ($rows[0]['title_1'] && $rows[0]['field_other_reception_venue_on_o'] == 0): ?>
                                <dt class="col s12 m3 bold"><?php print t('Reception'); ?></dt>
                                <dd class="col s12 m9 v-gold-text"><?php echo isset($rows[0]['title_1']) ? $rows[0]['title_1'] . ' at ' . $rows[0]['field_reception_time'] : '--' ?></dd>
                            <?php else: ?>
                                <dt class="col s12 m3 bold"><?php print t('Reception'); ?></dt>
                                <dd class="col s12 m9 v-gold-text"><?php print isset($rows[0]['field_other_reception_venue']) ? $rows[0]['field_other_reception_venue'] . ' at ' . $rows[0]['field_reception_time'] : ' '  ?></dd>
                            <?php endif; ?>  

                            <?php if ($rows[0]['field_package']): ?>
                                <dt class="col s12 m3 bold"><?php print t('Package'); ?></dt>
                                <dd class="col s12 m9 v-gold-text"><?php print isset($rows[0]['field_package']) ? $rows[0]['field_package'] : ' '  ?></dd>                    
                            <?php endif; ?> 
                            <!--dt class="col s12 m3 bold"><?php print t('Action'); ?></dt-->
                            <dd class="col s12 m9 v-gold-text">                    
                                <!--p><a class="v-gold-text bold" href="node/<?php print $nid; ?>/edit"><?php print t('Update Event'); ?></a></p-->                         
                            <?php } else { ?>
                            <dt class="col s12 m3 bold"><?php print t('Event'); ?></dt>
                            <dd class="col s12 m9 v-gold-text">                    
